...
@custom_heading_regexp = ();    # Add a regexp for headings.
                                ...
                                # subexpression, the first one will be
                                # used as a style identifier.  A
                                # second one, if it exists, will
                                # indicate what text should be kept. 
                                # For example, "^(:+)(.*)" would match
                                # any line that started with a colon. 
                                # The style text would be the colons
                                # (however many there were), and the
                                # colons would then be removed from
                                # the line.  The styles are used to
                                # determine the heading level.  The
                                # first style found in the document is
                                # level 1, the second level 2, etc.
...
sub liststuff
{
    ...
    for($i = $listnum - 1; ($prefix ne $listprefix[$i]) && ($i >= 0); $i--)
    {
...
sub custom_heading
{
    ...
    for($i=0; $i <= $#custom_heading_regexp; $i++)
    {
        ...
        {
            ($style, $rest) = ($line =~ /$custom_heading_regexp[$i]/);
            $line = ($rest . "\n") if $rest;
            $style || ($style = $i);
            &tagline("H" . &heading_level("Cust" . $style));
            &anchor_heading($rest);
            $line_action |= $HEADER;
            last;
        }
...
sub main
{
	...
	print "<TITLE>$title</TITLE>\n" if($title);
        ...
        $version = '$Revision: 1.21 $ ';  $version =~ s/.*(\d+\.\d+).*/$1/;
