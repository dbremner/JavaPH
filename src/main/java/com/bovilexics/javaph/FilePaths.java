package com.bovilexics.javaph;

import org.jetbrains.annotations.NotNull;

public class FilePaths
{
    public static final @NotNull String HELP_FILE_PATH = "help/index.html";
    public static final @NotNull String IMG_PH_ICON_SMALLER_GIF_FILEPATH = "img/ph-icon-smaller.gif";
    public static final String IMG_CUT_GIF_FILEPATH = "img/cut.gif";
    public static final String IMG_COPY_GIF_FILEPATH = "img/copy.gif";
    public static final String IMG_PASTE_GIF_FILEPATH = "img/paste.gif";
}
